...
func SendFile(c echo.Context) error {
	...
	dst, err := os.Create("uploadfiles/" + file.Filename)
	if err != nil {
		return err
	}
	defer dst.Close()
	...
	return c.String(http.StatusOK, fmt.Sprintf("success upload file: %v", file.Filename))
}

func ReadFile(c echo.Context) error {
	return c.String(http.StatusOK, "read file")
	// return nil
}
